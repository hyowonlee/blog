...
public class UserApiController {
    ...
    public ResponseDto<Integer> login(@RequestBody User user, HttpSession session) //위에 di된 session객체 있지만 여기선 그냥 매개변수로 세션 받아봄 세션에 대한 설명은 밑에 적힘
    {
        System.out.println("UserApiController.login()");
        User principal = userService.login(user); //principal은 접근주체라는 용어로 많이 쓰인다

        if(principal != null) // 해당 사용자가 있으면 세션을 만들어주면 되는데 세션은 스프링에서 컨트롤러의 매개변수로 적어주면 자동으로 받을 수 있고 또는 세션객체는 스프링 컨테이너가 빈등록해 가지고있으니 @Autowired해서 받을 수 있다 (HtpSession)
        {
