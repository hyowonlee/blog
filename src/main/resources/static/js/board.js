var index = { // javascript 객체 ajax때문에 페이지마다 여러번 선언되어서 같은이름 여러번 선언 가능한 var로 했음 let은 안됨
    ...
    update: function () {
        ...
        $.ajax({
            ...
            if (response.data == 1) {
                ...
                // 글 수정 완료 후 ajax로 페이지 갈려고 여기서 다시 ajax 요청해서 페이지 가져와서 장갈이 함
                $.ajax({
                    method: 'GET',
                    url: '/auth/board/' + id,
                    dataType: 'html',
    ...
    // 댓글 업데이트
    replyUpdate: function (boardId, replyId, content) {
        let data = {
            id: replyId,
            content: content
        };
        ...
        $.ajax({
            type: 'PUT',
            url: `/api/board/${boardId}/reply/update/${data.id}`,
            data: JSON.stringify(data),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
        }).done(function (response) { // ajax요청해서 받은 응답이 여기 매개변수로 들어옴
    ...
    //댓글 수정버튼 클릭시 밑에 댓글창 나오기
    replyAppend: function (boardId, replyId) {
        $("[id^=reply-update-]").remove(); // 선택자 안에선 *인 와일드카드가 안먹히길래 찾아보니 정규표현식으로 표현해줘야함, 지금 나와있는 다른 댓글작성창 지우기
        $("#reply-"+replyId).after(`<li id="reply-update-${replyId}" class="list-group-item d-flex"></li>`);
        $("#reply-update-"+replyId).append(`<div class='card-body'>
                <textarea id='reply-update-content${replyId}' class='form-control' rows='2'></textarea>
                <br />
                <button class="float-right badge" onclick="index.replyUpdate(${boardId}, ${replyId}, document.getElementById('reply-update-content${replyId}').value)">등록</button>
            </div>
            `);
    }
}
